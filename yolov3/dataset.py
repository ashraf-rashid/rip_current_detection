            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    try:
                        ...
                        exception_index.append(num)
                        
                    num += 1
                if exceptions: 
                    print('\n')
                    print("There were problems with dataset, I fixed them, now do not restart the training process.")
                    print("Deleting faulty indexes")
                    
                    #Delete faulty indexes from current batch
                    batch_image = np.delete(batch_image, exception_index, axis=0)
                    batch_label_sbbox = np.delete(batch_label_sbbox, exception_index, axis=0)
                    batch_label_mbbox = np.delete(batch_label_mbbox, exception_index, axis=0)
                    batch_label_lbbox = np.delete(batch_label_lbbox, exception_index, axis=0)
                    batch_sbboxes = np.delete(batch_sbboxes, exception_index, axis=0)
                    batch_mbboxes = np.delete(batch_mbboxes, exception_index, axis=0)
                    batch_lbboxes = np.delete(batch_lbboxes, exception_index, axis=0)
