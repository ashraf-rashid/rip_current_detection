...
def darknet19_tiny(input_data):
    ...
    bn = True
    #convolutional-1
    conv1 = Conv2D(filters=16, kernel_size = 3, strides=strides,
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.))(input_data) #(3,3,3,16)
    bn1= BatchNormalization()(conv1)
    #relu1= LeakyReLU(alpha=0.1)(bn1)
    relu1= mish(bn1)
    mp1 = MaxPool2D(2, 2, 'same')(relu1)   
    #input_data = MaxPool2D(2, 2, 'same')(input_data)
    

    #input_data = convolutional(input_data, (3, 3, 16, 32))
    conv2 = Conv2D(filters=32, kernel_size = 3, strides=strides,
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.))(mp1) #(3,3,16,32)
    bn2 = BatchNormalization()(conv2)
    
    #residual connection-1
    conv_res1 = Conv2D(filters=32, kernel_size = 3, strides=(2,2),
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.),name='conv_res1')(relu1)
    bn_res1 = BatchNormalization(name='bn_res1')(conv_res1)
    add_res1 = Add(name='add_res1')([bn2,bn_res1])
    
    #activate-2
    #relu2= LeakyReLU(alpha=0.1)(add_res1)
    relu2= mish(add_res1)
    mp2 = MaxPool2D(2, 2, 'same')(relu2)
    #input_data = MaxPool2D(2, 2, 'same')(input_data)
    ...
    #input_data = convolutional(input_data, (3, 3, 32, 64))
    #convolutional-3
    conv3 = Conv2D(filters=64, kernel_size = 3, strides=strides,
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.))(mp2) #(3,3,32,64)
    bn3 = BatchNormalization()(conv3)
    
    #residual connection-2
    conv_res2 = Conv2D(filters=64, kernel_size = 3, strides=(2,2),
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.),name='conv_res2')(relu2)
    bn_res2 = BatchNormalization(name='bn_res2')(conv_res2)
    add_res2 = Add(name='add_res2')([bn3,bn_res2])
    ...
    # input_data = MaxPool2D(2, 2, 'same')(input_data)
     ...
     #activate-3
    #relu3 = LeakyReLU(alpha=0.1)(add_res2)
    relu3 = mish(add_res2)
    mp3 = MaxPool2D(2, 2, 'same')(relu3)    
    #input_data = MaxPool2D(2, 2, 'same')(input_data)
    
    #input_data = convolutional(input_data, (3, 3, 64, 128))
    #convolutional-4
    conv4 = Conv2D(filters=128, kernel_size = 3, strides=strides,
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.))(mp3) #(3,3,64,128)
    bn4 = BatchNormalization()(conv4)
    
    #residual connection-3
    conv_res3 = Conv2D(filters=128, kernel_size = 3, strides=(2,2),
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.),name='conv_res3')(relu3)
    bn_res3 = BatchNormalization(name='bn_res3')(conv_res3)
    add_res3 = Add(name='add_res3')([bn4,bn_res3])
    
    #activate-4
    #relu4 = LeakyReLU(alpha=0.1)(add_res3)
    relu4 = mish(add_res3)
    mp4 = MaxPool2D(2, 2, 'same')(relu4)
    #input_data = MaxPool2D(2, 2, 'same')(input_data)
    ...
    #input_data = convolutional(input_data, (3, 3, 128, 256))
    #convolutional-5
    conv5 = Conv2D(filters=256, kernel_size = 3, strides=strides,
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.))(mp4) #(3,3,128,256)
    bn5 = BatchNormalization()(conv5)
    
    #residual connection-4
    conv_res4 = Conv2D(filters=256, kernel_size = 3, strides=(2,2),
                  padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  bias_initializer=tf.constant_initializer(0.), name='conv_res4')(relu4)
    bn_res4 = BatchNormalization(name='bn_res4')(conv_res4)
    add_res4 = Add(name='add_res4')([bn5,bn_res4])
    #relu5 = LeakyReLU(alpha=0.1)(add_res4)
    relu5 = mish(add_res4)
    ...
    input_data = MaxPool2D(2, 2, 'same')(relu5)
    input_data = convolutional(input_data, (3, 3, 256, 512))
    input_data = MaxPool2D(2, 1, 'same')(input_data)
    input_data = convolutional(input_data, (3, 3, 512, 1024))
    ...
    # #input_data = MaxPool2D(2, 2, 'same')(input_data)
    # mp5 = MaxPool2D(2, 2, 'same')(relu5)
    
    # #input_data = convolutional(mp5, (3, 3, 256, 512))
    # #convolutional-6
    # conv6 = Conv2D(filters=512, kernel_size = 3, strides=strides,
                  # padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  # kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  # bias_initializer=tf.constant_initializer(0.))(mp5) #(3,3,256,512)
    # bn6 = BatchNormalization()(conv6)  


    # #residual connection-5
    # conv_res5 = Conv2D(filters=512, kernel_size = 3, strides=(2,2),
                  # padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  # kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  # bias_initializer=tf.constant_initializer(0.), name='conv_res5')(relu5)
    # bn_res5 = BatchNormalization(name='bn_res5')(conv_res5)
    # add_res5 = Add(name='add_res5')([bn6,bn_res5])
    # relu6 = LeakyReLU(alpha=0.1)(add_res5)
    
    # #input_data = MaxPool2D(2, 1, 'same')(input_data)
    # mp6 = MaxPool2D(2, 1, 'same')(relu6)
    
    # #input_data = convolutional(input_data, (3, 3, 512, 1024))
    # #convolutional-7
    # conv7 = Conv2D(filters=1024, kernel_size = 3, strides=strides,
                  # padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  # kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  # bias_initializer=tf.constant_initializer(0.))(mp6) #(3,3,512,1024)             
    # bn7 = BatchNormalization()(conv7)  
    
    
    # #residual connection-6
    # conv_res6 = Conv2D(filters=1024, kernel_size = 1, strides=strides,
                  # padding=padding, use_bias=not bn, kernel_regularizer=l2(0.0005),
                  # kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                  # bias_initializer=tf.constant_initializer(0.), name='conv_res6')(relu6)
    # bn_res6 = BatchNormalization(name='bn_res6')(conv_res6)
    # add_res6 = Add(name='add_res6')([bn7,bn_res6])
    # relu7 = LeakyReLU(alpha=0.1)(add_res6)
    
    #input_data = relu7
    
    print('finished la la la')
...
def YOLOv3_tiny(input_layer, NUM_CLASS):
    ...
    #conv_lbbox = convolutional(conv_lobj_branch, (1, 1, 512, 3*(NUM_CLASS + 5)), activate=True, bn=False)
    ...
    #conv_mbbox = convolutional(conv_mobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=True, bn=False)
