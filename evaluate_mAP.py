...
def get_mAP(Yolo, dataset, score_threshold=0.25, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
    ...
    for index in range(dataset.num_samples):
        ...
        # x = pred_bbox[0]
        # p1 = tf.reshape(x, (-1, tf.shape(x)[-1]))
        
        # x = pred_bbox[1]
        # temp = tf.reshape(x, (-1, ))
        
        #pred_bbox = [tf.reshape(x, (-1, tf.shape(x)[-1])) for x in pred_bbox]
        pred_bbox = [tf.reshape(x, (-1, 6)) for x in pred_bbox]
    ...
    with open("mAP/results.txt", 'w') as results_file:
        ...
        with open("D:/Imran_Razzak/TensorFlow-2.x-YOLOv3/mAP_resripdet1_final_activation_true.txt","a") as f:
            f.write(text + "\n" + "******" + "\n")
...
def get_mAP2(Yolo, dataset, score_threshold=0.25, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
    MINOVERLAP = 0.5 # default value (defined in the PASCAL VOC2012 challenge)
    NUM_CLASS = read_class_names(TRAIN_CLASSES)
    ...
    ap_dictionary = {}
    # open file to store the results
    with open("D:/Imran_Razzak/TensorFlow-2.x-YOLOv3/mAP_results_resripdet1_mish_activation.txt", 'w') as results_file:
        results_file.write("# AP and precision/recall per class\n")
        count_true_positives = {}
        for class_index, class_name in enumerate(gt_classes):
            count_true_positives[class_name] = 0
        ...
        with open("D:/Imran_Razzak/TensorFlow-2.x-YOLOv3/mAP_resripdet1_mish_activation.txt","a") as f:
            f.write(text + "\n" + "******" + "\n")
