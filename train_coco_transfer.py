...
def main():
    ...
    if TRAIN_TRANSFER:
        ...
        Darknet = tf.keras.models.load_model('Yolo_coco_trained_20_epochs.hdf5')
        Darknet.load_weights('D:/Imran_Razzak/TensorFlow-2.x-YOLOv3/model_data/from_imran_pc/yolov3_custom_Tiny_val_loss_  37.75')
    ...
    #print('created resripdet1')
    #print(yolo.summary())
    #yolo = load_model('test_resripdet1_swa_80.hdf5')
        ...
        with tf.GradientTape() as tape:
            ...
            if global_steps < warmup_steps:# and not TRAIN_TRANSFER:
                ...
                lr = TRAIN_LR_END + 0.5 * (TRAIN_LR_INIT - TRAIN_LR_END)*(
                    (1 + tf.cos((global_steps - warmup_steps) / (total_steps - warmup_steps) * np.pi))) #(total_steps - warmup_steps) -> 50
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        if TRAIN_SAVE_CHECKPOINT and not TRAIN_SAVE_BEST_ONLY:
            save_directory = os.path.join(TRAIN_CHECKPOINTS_FOLDER, TRAIN_MODEL_NAME+"_"+str(epoch) + "_val_loss_{:7.2f}".format(total_val/count))
    ...
    mAP_model.load_weights(save_directory) # use keras weights
    get_mAP(mAP_model, testset, score_threshold=TEST_SCORE_THRESHOLD, iou_threshold=TEST_IOU_THRESHOLD)
    yolo.save('Tiny-yolo-object-twice-data-aug-resripdet1-TL-coco-100ep.hdf5')
